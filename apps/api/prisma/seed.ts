import { PrismaClient, GoalMetric, GoalPeriod, SupplyType } from '@prisma/client';
import * as bcrypt from 'bcryptjs';

const prisma = new PrismaClient();

async function seedDemo(shopId: string) {
  const demoProducts = [
  ...
  for (const item of demoProducts) {
    ...
    if (!await prisma.skuCost.findFirst({ where: { skuId: sku.id, validTo: null } })) {
      await prisma.skuCost.create({ data: { skuId: sku.id, amount: item.cost, warehouseLogisticsCost: 1500, createdBy: 'demo-seed' } });
    }
  }

  if (!await prisma.supply.count({ where: { shopId } })) {
  ...
  if (!await prisma.goal.count({ where: { shopId } })) {
    ...
    await prisma.goal.createMany({ data: [
      { shopId, metric: GoalMetric.REVENUE, period: GoalPeriod.WEEKLY, targetValue: 25_000_000, startAt, endAt, note: 'Демо-цель' },
    ] });
  }
}
...
async function main() {
  ...
  const shopName = process.env.DEFAULT_SHOP_NAME?.trim() || existingShop?.name || `Uzum shop ${externalId}`;
