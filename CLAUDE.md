...
- **Business logic is extracted into pure functions in `src/common/`** (`finance.ts`, `payout-basket.ts`, `payout-forecast.ts`, `order-state.ts`, `advertising.ts`, `inventory-report.ts`, `inventory-analytics.ts`, `sku-payout.ts`, `profit-days.ts`, `financial-statement.ts`, `fbo-supply-summary.ts`, `period.ts`, `pricing.ts`, `promo-pricing.ts`, `auto-pricing.ts`, `ai-price-review.ts`, `ad-agent.ts`). Services orchestrate Prisma + these functions; put testable logic here, not in services.
...
- Price changes: `PricingService.sendPrice` / `apps/api/scripts/send-price.ts` (dry-run by default, every attempt logged in `PriceChange`). Uzum rejects `sendPriceData` with `sku-price-001` while a SKU is in an Uzum promo; promo prices are only editable in the seller cabinet, and the OpenAPI `price` field is the promo price during a promo, not the base price. Promo prices go through the cabinet's internal API at `https://api.uzum.uz/api/seller/shop/{shopId}/marketing/sales/...` with the `UZUM_INTERNAL` token: `PromoPricingService` / `apps/api/scripts/send-promo-price.ts` (`--list` to read base price, promo price and the "не более" limit; dry-run by default; logged in `PriceChange` with `kind = PROMO`). Saving posts the whole product (all its SKUs in the sale), exactly like the cabinet does.
- Auto-pricing: rules in `common/auto-pricing.ts` (roles LOCOMOTIVE = лицевые incl. HAVANA 50×90 / MARGINAL, deficit/flow/revert/slow rules, promo "не более" limit → recommendation "поднять базовую после акции", sales and margin by buyouts, not orders). `AutoPricingService` runs at 09:30 and 19:30 Asia/Tashkent (`AUTO_PRICING_CRON`), reads stock days from the cabinet `GET https://api-seller.uzum.uz/api/seller/shop/{shopId}/product/getProducts` (`avgdsales`, `turnover`, `forecastOutOfStock`) and sends a Telegram report. It changes prices only with `AUTO_PRICING_APPLY=true` in `.env` (default: recommendations only), through `sendPrice` / `sendPromoPrice` with `source = auto`, `PriceChange.rule` and `PriceChange.context` — the rules read that journal back (one change per SKU per day, cooldowns, revert after supply / sales drop). Before applying, every rule-proposed change is reviewed by Claude via `OpenclawClient` (`common/ai-price-review.ts`): the AI may only approve, reject or soften (price between current and proposed) — never add changes; if the AI is unavailable in apply mode, nothing is changed (`AUTO_PRICING_AI=false` disables the review). Manual report: `apps/api/scripts/auto-pricing.ts` (`--telegram`, `--dump-stock`) or Actions → «Автоцены».
- Ad agent: `AdAgentService` (`modules/ads`, rules in `common/ad-agent.ts`) runs daily at 10:00 Asia/Tashkent (`AD_AGENT_CRON`) and only sends advice to Telegram (target: reach and traffic for slow sellers, ДРР ≤ `AD_AGENT_MAX_DRR_PERCENT`, default 10). Data via the `UZUM_INTERNAL` Bearer token: `getProducts`, `POST /api/seller/cpo/advertisements/search` («Буст заказов» bids), `GET /api/seller/advertising/management/ad-campaign` («Буст в ТОП»), and Cube.js `https://analytics-seller.uzum.uz/cubejs-api/v1/load` (cubes `SellerReportProductFunnelSku`, `AdvertisingCpoFunnel`, `AdvertisingDailyFunnel`; retry on `Continue wait`). Write endpoints for bids/budgets are not captured yet. Both agents notify via the Telegram toggle `notifyAgents` («Автоцены и реклама»). Manual run: `apps/api/scripts/ad-agent.ts` or Actions → «Реклама»; both agent workflows first print server version, deploy log tail and the API journal lines of the agents.
